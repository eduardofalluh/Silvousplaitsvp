...
exports.handler = async (event) => {
  ...
  if (!normalize(body.region)) {
    return { statusCode: 400, headers, body: JSON.stringify({ error: 'La region est requise' }) };
  }

  try {
    const result = await savePremiumOffer(body);
    return {
      statusCode: 200,
      headers,
    ...
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({ error: error.message || 'Failed to save premium offer' }),
    };
  }
};
