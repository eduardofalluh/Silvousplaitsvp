// Admin: create/update an archived premium offer (edit an offer in the archive tab).
...
exports.handler = async (event) => {
  ...
  if (!normalize(body.title)) return { statusCode: 400, headers, body: JSON.stringify({ error: 'Le titre est requis' }) };

  try {
    const result = await saveArchivedOffer(body);
    return { statusCode: 200, headers, body: JSON.stringify({ success: true, ...result }) };
  } catch (error) {
    return { statusCode: 500, headers, body: JSON.stringify({ error: error.message || 'Failed to save archived offer' }) };
  }
};
