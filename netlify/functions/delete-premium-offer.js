...
exports.handler = async (event) => {
  ...
  try {
    const result = await deletePremiumOffer(body.id);
    return {
      statusCode: 200,
      headers,
    ...
    return {
      statusCode: error.message === 'Offer not found' ? 404 : 500,
      headers,
      body: JSON.stringify({ error: error.message || 'Failed to delete premium offer' }),
    };
  }
};
