...
exports.handler = async (event) => {
  ...
  try {
    const result = await deleteFreeSignupLocation(body.id);
    return {
      statusCode: 200,
      headers,
    ...
    const isConflict = error.message === 'Impossible de supprimer la derniere ville';
    return {
      statusCode: isNotFound ? 404 : isConflict ? 409 : 500,
      headers,
      body: JSON.stringify({ error: error.message || 'Failed to delete free signup location' }),
    };
  }
};
